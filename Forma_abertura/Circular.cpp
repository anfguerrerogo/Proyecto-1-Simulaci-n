//Simulación Reloj de Arena (30x13 partículas)
#include <iostream>
#include <fstream>
#include <cmath>
#include "Vector.h"
...
int main(void){
  ...
  Colisionador Newton;
  Crandom ran64(1); 
  double Numpartabajo=0, Flowrate=0, Numpart=0;
  int Tint=0;
  double tdibujo=0;
  ...
  for(t=tdibujo=0;t<tmax;t+=dt,tdibujo+=dt){
    ...
    if(Tint % 50 == 0) Flujo(Particula,  Numpart, Flowrate, Numpartabajo, t);
    for(int i=0;i<N;i++){
      if(Particula[i].Gety()<-1)Particula[i].Inicie((Lx-2*R0)*ran64.r(), Ly-2*R0, 0, 0, 0, 0, 0, 0, m0, R0);
    }
    if(Numpartabajo==30*N) break;
  }
  return 0;
}
