//Simulación Reloj de Arena con interacción electrostática entre partículas
#include <iostream>
#include <fstream>
#include <cmath>
#include "Vector.h"
...
//---------------------Constantes globales--------
const double g=9.8;
const double K=1e4, Gamma=50, Kcundall=10, MU=0.4, k=9e3; //Falta arreglar unidades
const double Lx=100,Ly=150;               //Tamaño 
const int Nx=2,Ny=1,N=Nx*Ny;   //Número partículas
const double Zeta=0.1786178958448091;
const double Lambda=-0.2123418310626054;
const double Xi=-0.06626458266981849;
const double Rpared=10000,Mpared=1000, Rparedcircular=0.45*Lx;//Propiedades paredes
...
class Cuerpo{
private:
  vector3D r,V,F,omega,tau; double m,R,theta,I,q;
public:
  void Inicie(double x0,double y0,double z0,
	      double Vx0,double Vy0,double Vz0,
	      double theta0,double omega0,
	      double m0,double R0, double q0);//Se añade carga al cuerpo
...
void Cuerpo::Inicie(double x0, double y0, double z0,
  ...
  theta=theta0;  m=m0; R=R0; I=2.0/5*m*R*R; q=q0;
}
...
void Colisionador::CalculeLaFuerzaEntre(Cuerpo & Particula1,Cuerpo & Particula2,
  ...
  //Fuerza Coulomb
  vector3D Felec;
  Felec=k*Particula2.q*Particula1.q/norma2(r21)*(r21/d21);
  Particula2.AgregueFuerza(Felec);      
  Particula1.AgregueFuerza(Felec*(-1));
}
...
void InicieCuadro(void){
    ...
    cout<<" , "<<Lx/7<<"*t,150";//Pared arriba
    cout<<" , 0,"<<Ly/7<<"*t"; //Pared izquierda
    cout<<" , 100,"<<Ly/7<<"*t";//Pared derecha
    cout<<", "<<Rparedcircular<<"*cos(t),"<<Rparedcircular<<"*sin(t)";//Dibuja pared circular izquierda
    cout<<", "<<Lx<<"+"<<Rparedcircular<<"*cos(t),"<<Rparedcircular<<"*sin(t)";//Dibuja pared circular derecha
}
...
int main(void){
  ...
  double m0=1,R0=1.5,V=10,omega0=10, carga=1;//Propiedades partículas

  double T=Lx/V, teq=10*T, tmax=2*T;//Variables temporales
  double DeltaLy=Ly/8;//Fracción de la caja donde comienzan las partículas
 //Se calcula espacio entre partículas para que queden espaciadas y no se sobrelapen (se requiere Nx<Lx/(2*R0), Ny<DeltaLy/(2*R0))
  ...
  //PAREDES
  //---------------(x0  ,       y0,z0,Vx0,Vy0,Vz0,theta0,omega0,    m0,     R0, q);
